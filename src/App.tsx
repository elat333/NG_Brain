...
const getModuleAccess = (
  ...
  // Handle process_dashboard (GestiÃ³n XD) module special case
    ...
    >
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-auto">
        ...
        <div className="flex items-center gap-2">
          {task.dueDate && (
            <span className="text-[9px] font-bold text-red-500/90 bg-red-50 px-1.5 py-0.5 rounded flex items-center gap-1" title="Fecha LÃ­mite">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="Ej: LÃ­der de Operaciones"
          />
        </div>
...
export default function App() {
    ...
    const deleteOrphans = async () => {
      try {
        ...
        snap.docs.forEach(async (d) => {
          const data = d.data();
          if (data.title?.includes('DiseÃ±o de artes para anuncios - ASÃ virtual') || data.id !== d.id) {
  ...
  const migrateFromLocalStorage = async () => {
    if (!user) return;
    if (!window.confirm('Se han detectado datos guardados en este navegador. Â¿Deseas migrarlos a tu cuenta de Novagreen IA? Esto no borrarÃ¡ los datos tÃ©cnicos, pero subirÃ¡ tu informaciÃ³n a la nube.')) return;
    ...
    try {
      ...
      if (migratedCount > 0) {
        alert(`${migratedCount} elementos migrados correctamente a la nube. El sistema se actualizarÃ¡ ahora.`);
        setLocalDataFound(false);
      } else {
        alert('No se encontraron datos estructurados compatibles para migrar. Intenta cargar los datos iniciales si la cuenta estÃ¡ vacÃ­a.');
      }
    } catch (error) {
  ...
  const [newCompanyData, setNewCompanyData] = useState({
    ...
    phone: '',
    website: '',
    mainAddress: '', // Matrix address (DirecciÃ³n Matriz)
  ...
  const deleteSuggestedActivity = (id: string) => {
    if (window.confirm('Â¿Realmente deseas borrar esta actividad sugerida?')) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      alert('Hubo un error al analizar la transcripciÃ³n.');
    } finally {
  ...
  const handleDeleteCompany = async (id: string) => {
    if (window.confirm('Â¿EstÃ¡s seguro de que deseas eliminar esta compaÃ±Ã­a?')) {
      try {
  ...
  const handleImportTasks = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        alert('Error al importar tareas. AsegÃºrese de que sea un archivo JSON vÃ¡lido.');
      }
    };
  ...
  const handleUpdateTask = async (e: React.FormEvent) => {
    ...
    if (statusChanged && (newTaskData.status === 'done' || newTaskData.status === 'correction')) {
      ...
      if (selectedRevisorId) {
        if (!currentMember || (currentMember.id !== selectedRevisorId && !isProcessLeader && !isUserAdmin)) {
          ...
          alert(`Error: Solo el Revisor designado (${revisorName}) o un lÃ­der/administrador puede pasar esta tarea a Completada o enviarla a CorrecciÃ³n.`);
          return;
        }
      } else {
        if (!isProcessLeader && !isUserAdmin) {
          alert('Error: Solo un lÃ­der de proceso o administrador puede pasar la tarea a Completada o enviarla a CorrecciÃ³n.');
          return;
        }
      }
    }
    ...
    try {
      ...
      if (editingTask.title !== newTaskData.title) {
        changes.push(`ModificÃ³ el tÃ­tulo a "${newTaskData.title}"`);
      }
      if (editingTask.status !== newTaskData.status) {
        ...
        changes.push(`CambiÃ³ el estado de "${oldState}" a "${newState}"`);
      }
      if ((editingTask.description || '') !== (newTaskData.description || '')) {
        changes.push('ModificÃ³ la descripciÃ³n');
      }
      if ((editingTask.storyDescription || '') !== (newTaskData.storyDescription || '')) {
        changes.push('ModificÃ³ la descripciÃ³n de la historia');
      }
      if ((editingTask.acceptanceCriteria || '') !== (newTaskData.acceptanceCriteria || '')) {
        changes.push('ModificÃ³ los criterios de aceptaciÃ³n');
      }
      if (editingTask.memberId !== newTaskData.memberId) {
        ...
        changes.push(`CambiÃ³ el responsable de "${prevMember}" a "${nextMember}"`);
      }
      if ((editingTask.plannedDate || '') !== (newTaskData.plannedDate || '')) {
        changes.push(`ModificÃ³ inicio planificado a ${newTaskData.plannedDate || 'Sin fecha'}`);
      }
      if ((editingTask.plannedEndDate || '') !== (newTaskData.plannedEndDate || '')) {
        changes.push(`ModificÃ³ fin planificado a ${newTaskData.plannedEndDate || 'Sin fecha'}`);
      }
      if ((editingTask.dueDate || '') !== (newTaskData.dueDate || '')) {
        changes.push(`ModificÃ³ fecha de entrega a ${newTaskData.dueDate || 'Sin fecha'}`);
      }
  ...
  const updateTaskStatus = async (id: string, newStatus: Task['status']) => {
    ...
    if (newStatus === 'in_progress') {
      ...
      if (isBlocked) {
        alert(`ESTA TAREA ESTÃ BLOQUEADAPara poder iniciar esta tarea se debe terminar primero:â€¢ ${blockers.map(t => t.title).join('â€¢ ')}`);
        return;
      }
    }
    try {
      ...
      const statusHistoryItem = {
        ...
        details: `CambiÃ³ el estado de "${oldState}" a "${newState}"`
      };
  ...
  const handleAddProject = async (e: React.FormEvent) => {
    ...
    if (access !== 'lider' && access !== 'administrador') {
      alert('Error: No dispones de privilegios de lÃ­der o administrador para crear proyectos en este proceso.');
      return;
    }
  ...
  const deleteProject = async (id: string) => {
    ...
    if (window.confirm('Â¿EstÃ¡s seguro de que deseas eliminar este proyecto? Las tareas asociadas perderÃ¡n su vinculaciÃ³n con el proyecto.')) {
      try {
  ...
  const confirmDeleteTask = async () => {
    ...
    if (taskAccess !== 'lider' && taskAccess !== 'administrador') {
      alert('Error: Solo los LÃ­deres de este Proceso o Administradores pueden eliminar tareas.');
      ...
      <div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-ng-black/40 font-bold mb-10 leading-relaxed uppercase text-xs tracking-widest">
            La plataforma inteligente para la gestiÃ³n de equipos y optimizaciÃ³n de procesos. 
            Inicia sesiÃ³n con tu cuenta corporativa para continuar.
          </p>
          <button 
            type="button"
          ...
          <div className="mt-12 pt-8 border-t border-gray-50 space-y-4">
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-loose">
              Al ingresar aceptas nuestros <a href="#" className="text-ng-green hover:underline">tÃ©rminos de servicio</a> y <a href="#" className="text-ng-green hover:underline">polÃ­tica de privacidad</a>.
            </p>
          </div>
        </motion.div>
      </div>
  ...
  return (
                  ...
                  {expandedNavModule === 'capacitacion' && (
                    ...
                    >
                      ...
                      <SubNavButton 
                        ...
                        label="GestiÃ³n de Capacitaciones" 
        ...
        <div className="mt-auto p-3 border-t border-ng-gray/10 shrink-0">
          ...
          {members.length === 0 && (
            <div className="flex flex-col gap-1 mt-1.5 px-1">
              ...
              >
                {isInitializingData ? 'Inicializando...' : 'Â¿Sin datos? Cargar iniciales'}
              </button>
              {localDataFound && (
                <button 
        ...
        <header className={`flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white text-ng-black border-b border-gray-100 shadow-2xs relative z-30 transition-all ${
          ...
          <div>
            <h1 className={`font-black tracking-tight ${['tasks', 'gerencia', 'marketing', 'ventas', 'capacitacion', 'acreditacion', 'qhse', 'importaciones', 'process_dashboard', 'productos'].includes(activeTab) ? 'text-lg md:text-xl' : 'text-2xl'}`}>
              ...
              {activeTab === 'gerencia' && 'MÃ³dulo de Gerencia & DirecciÃ³n'}
              {activeTab === 'process_dashboard' && 'GestiÃ³n de Procesos'}
              {activeTab === 'importaciones' && 'MÃ³dulo de Importaciones'}
              {activeTab === 'marketing' && 'MÃ³dulo de Marketing'}
              {activeTab === 'ventas' && 'MÃ³dulo de Ventas'}
              {activeTab === 'capacitacion' && 'MÃ³dulo de CapacitaciÃ³n'}
              {activeTab === 'acreditacion' && 'MÃ³dulo de AcreditaciÃ³n'}
              {activeTab === 'productos' && 'MÃ³dulo de Productos'}
              {activeTab === 'qhse' && 'MÃ³dulo de QHSE'}
              {activeTab === 'transcript' && 'AnÃ¡lisis de Transcripciones'}
              {activeTab === 'projects' && 'GestiÃ³n de Proyectos'}
              {activeTab === 'tasks' && 'Seguimiento de Tareas'}
              {activeTab === 'planner' && 'Asistente de PlanificaciÃ³n'}
              {activeTab === 'directory' && (
                directorySubTab === 'people' ? 'Directorio de Personas' : 
                directorySubTab === 'companies' ? 'Directorio de CompaÃ±Ã­as' : 
                'Industrias y Sectores'
              )}
              {activeTab === 'settings' && (
                settingsSubTab === 'roles' ? 'Permisos por Integrante' :
                settingsSubTab === 'processes' ? 'GestiÃ³n de Procesos' :
                settingsSubTab === 'members' ? 'GestiÃ³n de Equipo' :
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-teal-50 text-teal-700 rounded-lg text-[11px] font-bold border border-teal-100/60 shadow-xs">
                        ...
                        <span className="capitalize text-[11px]">
                          Est: {
                            ...
                            smartFilters.status === 'review' ? 'En RevisiÃ³n' :
                    ...
                    <input 
                      ...
                      placeholder={
                        Object.values(smartFilters).some(v => v !== null)
                          ? "Filtros activos. Buscar mÃ¡s o filtrar..."
                    ...
                    {activeSuggestionCategory === 'all' && (
                      <div className="p-2.5">
                        <div className="px-3 py-1.5 border-b border-gray-50 flex items-center justify-between mb-1.5">
                          <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">Buscar o filtrar por categorÃ­a</span>
            ...
            {activeTab === 'directory' && !isReadOnly && (
              <div className="flex items-center gap-4">
                {directorySubTab !== 'industries' && (
                  ...
                  >
                    <Plus size={18} />
                    {directorySubTab === 'people' ? 'Nueva Persona' : 'Nueva CompaÃ±Ã­a'}
                  </button>
                )}
              </div>
            )}
                ...
                <div id="restricted-card" className="max-w-md w-full bg-white p-8 rounded-[2rem] border border-gray-100 shadow-2xl shadow-gray-100 text-center space-y-6 relative overflow-hidden">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-medium text-slate-400">
                      No tienes los permisos requeridos para ingresar a este mÃ³dulo.
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-left space-y-3">
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider flex items-center gap-2">
                      <Lock size={12} className="text-red-400" /> InformaciÃ³n de Tu Acceso
                    </p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div className="text-slate-400 text-left">MÃ³dulo:</div>
                      <div className="font-extrabold text-slate-700 capitalize text-left">
                        {activeTab === 'dashboard' ? 'Resumen' :
                         ...
                         activeTab === 'transcript' ? 'Analizar ReuniÃ³n' :
                         'ConfiguraciÃ³n'}
                      </div>
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed font-medium">
                    Por favor, solicita a un Administrador o LÃ­der de Proceso que te asigne acceso en la <strong className="text-slate-600">Matriz de Permisos</strong> dentro de ConfiguraciÃ³n.
                  </p>
                </div>
              </motion.div>
            ) : (
            ...
            >
              ...
              <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 bg-white p-6 rounded-2xl border border-ng-gray shadow-sm relative overflow-hidden group">
                  ...
                  <div className="space-y-4">
                    <AIInsightItem 
                      title="ActualizaciÃ³n de Perfil"
                      desc="Elena Rodriguez aÃ±adiÃ³ 'GCP Architecture' a sus habilidades."
                      time="Hace 2 horas"
                    />
                    <AIInsightItem 
                      title="Nuevo Objetivo"
                      desc="El proceso de Desarrollo de Software tiene un nuevo objetivo de migraciÃ³n."
                      time="Hace 5 horas"
                    />
                    <AIInsightItem 
                      title="Logro Detectado"
                      desc="Lucas Smith completÃ³ la campaÃ±a SEO trimestral."
                      time="Ayer"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-ng-black text-white p-6 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
                  ...
                  <h3 className="text-lg font-black mb-2 tracking-tight">Â¿Nueva ReuniÃ³n?</h3>
                  <p className="text-ng-gray/60 text-sm mb-6 font-medium">Pega la transcripciÃ³n y deja que la IA actualice los perfiles automÃ¡ticamente.</p>
                  <button 
                  ...
                  {directorySubTab === 'people' ? (
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-gray-50">
                            ...
                            <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">CategorÃ­a</th>
                    ...
                    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-xl overflow-hidden">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-gray-50">
                            <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">CompaÃ±Ã­a</th>
                            ...
                            <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">DirecciÃ³n</th>
                            <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Contacto</th>
                            <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] text-right">Acciones</th>
                          </tr>
                        </thead>
                      ...
                      {companies.length === 0 && (
                        <div className="py-20 text-center">
                          ...
                          <p className="text-gray-400 font-medium">No hay compaÃ±Ã­as registradas.</p>
                        </div>
                      )}
                    </div>
                  ) : (
                        ...
                        return (
                          <div key={industry.id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
                             ...
                             <div className="flex items-start justify-between mb-4">
                               ...
                               <button 
                                 onClick={() => {
                                   if (window.confirm('Â¿Deseas eliminar este sector? Solo se eliminarÃ¡ de la lista maestra.')) {
                             ...
                             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                               <Building2 size={12} />
                               {associatedCompaniesCount} compaÃ±Ã­a(s) vinculada(s)
                             </p>
                          </div>
                        );
                      })}
              ...
              {!extractedUpdates ? (
                <div className="bg-white p-8 rounded-3xl border border-[#E5E7EB] shadow-xl">
                  ...
                  <textarea 
                    ...
                    placeholder="Pega aquÃ­ la transcripciÃ³n de la reuniÃ³n (Voz a Texto, Zoom, Teams, etc.)..."
                    value={transcript}
                  ...
                  <div className="mt-8 flex items-center justify-between">
                    <p className="text-xs text-gray-400 max-w-sm">
                      La IA buscarÃ¡ menciones de habilidades, cambios en roles, responsabilidades asignadas y actualizaciones de departamentos.
                    </p>
                    <button 
                    ...
                    <div className="space-y-4">
                      ...
                      {extractedUpdates.memberUpdates.length === 0 && (
                        <div className="p-8 bg-gray-50 rounded-2xl border border-dashed border-gray-200 text-center text-gray-400">
                          No se detectaron cambios para miembros especÃ­ficos.
                        </div>
                      )}
                              ...
                              {u.achievements && u.achievements.length > 0 && (
                                <div>
                                  ...
                                  <ul className="text-xs text-gray-600 space-y-1">
                                    {u.achievements.map((a, idx) => <li key={`${a}-${idx}`} className="flex gap-2"><span>â€¢</span> {a}</li>)}
                                  </ul>
                                </div>
                              )}
                            </div>
                        ...
                        return (
                          <div key={u.processId} className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-sm">
                            ...
                            <div className="space-y-3">
                              {u.descriptionUpdate && (
                                <div>
                                  <span className="text-[10px] uppercase font-bold text-purple-500 block mb-1">ActualizaciÃ³n de MisiÃ³n</span>
                              ...
                              {u.newGoals && u.newGoals.length > 0 && (
                                <div>
                                  ...
                                  <ul className="text-xs text-gray-600 space-y-1">
                                    {u.newGoals.map((g, idx) => <li key={`${g}-${idx}`} className="flex gap-2"><span>â€¢</span> {g}</li>)}
                                  </ul>
                                </div>
                              )}
                            </div>
              ...
              {tasksSubTab === 'permissions' ? (
                  ...
                  <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 mt-6">
                    <div className="flex items-center gap-4 mb-8 pb-6 border-b border-gray-100">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 mt-1">Niveles de acceso y responsabilidades en el MÃ³dulo de Tareas.</p>
                      </div>
                    </div>
                    ...
                    <div className="prose prose-sm md:prose-base prose-slate max-w-none space-y-8">
                      <section>
                        ...
                        <p className="text-gray-600 leading-relaxed">
                          El mÃ³dulo de seguimiento de tareas opera bajo un esquema de <strong>"Permisos Cruzados"</strong> que separa claramente la <em>PlanificaciÃ³n</em> de la <em>EjecuciÃ³n</em>, protegiendo asÃ­ el cronograma y presupuesto de los proyectos.
                        </p>
                      </section>

                      <section className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                        <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-4 flex items-center gap-2">
                          <CheckCircle2 size={16} className="text-green-500" /> LÃ­deres y Administradores
                        </h3>
                        <p className="text-sm text-gray-600 mb-4">
                          Tienen control total sobre el <strong>Bloque de PlanificaciÃ³n y LÃ­mites</strong>.
                        </p>
                        <ul className="space-y-2 text-sm text-gray-600">
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Establecer o modificar la <strong>Fecha LÃ­mite (Deadline)</strong>.</li>
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Asignar las <strong>Horas Planificadas</strong> (Presupuesto de tiempo).</li>
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Editar cualquier campo de la tarea y reasignar responsables.</li>
                        </ul>
                      </section>
                      ...
                      <section className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                          Es el dueÃ±o absoluto del <strong>Bloque de EjecuciÃ³n Real</strong>. 
                        </p>
                        <ul className="space-y-2 text-sm text-gray-600">
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Reportar las <strong>Horas Reales</strong> utilizadas en la tarea.</li>
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Actualizar la <strong>Fecha de entrega real</strong>. (El sistema la auto-llena al pasar a Completada).</li>
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> Agregar horarios especÃ­ficos (Hora Inicio / Hora Fin) al dÃ­a planificado para colaborar.</li>
                          <li className="flex items-start gap-2"><span className="text-blue-500 font-bold">â€¢</span> <em>Nota: Visualiza el bloque de planificaciÃ³n en modo "Solo lectura".</em></li>
                        </ul>
                      </section>
                      ...
                      <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <p className="text-sm text-gray-600">
                          Cualquier miembro del equipo que abra una tarea que <strong>NO</strong> tiene asignada, visualizarÃ¡ todos los campos en modo "Solo lectura". PodrÃ¡n ver los detalles, pero no podrÃ¡n modificar fechas ni horas.
                        </p>
                      </section>
                    </div>
                  </div>
                ...
                >
                  {[
                    ...
                    { id: 'review', label: 'En RevisiÃ³n', color: 'text-purple-500', bg: 'bg-purple-50' },
                    { id: 'correction', label: 'Para CorrecciÃ³n', color: 'text-amber-500', bg: 'bg-amber-50' },
                      ...
                      <div className={`flex-1 overflow-y-auto space-y-4 p-2 rounded-[2rem] ${column.bg}/30 border-2 border-dashed border-gray-100/50 hover:bg-white/40 transition-colors custom-scrollbar`}>
                        ...
                        {filteredTasks.filter(t => t.status === column.id).length === 0 && (
                          ...
                          >
                            ...
                            <span className="font-bold uppercase tracking-widest text-[9px]">AÃ±adir Tarea</span>
                          </button>
                        )}
                      </div>
                    </div>
                  ...
                  <table className="w-full text-left border-collapse min-w-[1250px]">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/50 sticky top-0 z-20 backdrop-blur-md">
                        {/* Tarea / DescripciÃ³n Column Header */}
                        ...
                        <th className="px-6 py-4 text-[10px] bg-gray-50/50 w-[14%] align-top border-r border-gray-100/50">
                          ...
                          <div className="relative">
                            ...
                            >
                              ...
                              <option value="review" className="font-sans text-purple-700 font-bold">En RevisiÃ³n</option>
                              <option value="correction" className="font-sans text-amber-700 font-bold">Para CorrecciÃ³n</option>
                          ...
                          <tr key={task.id} className="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td className="px-6 py-4.5 align-middle">
                              ...
                              >
                                ...
                                <option value="correction">Para CorrecciÃ³n</option>
                  ...
                  <div className="bg-white p-8 rounded-3xl border border-[#E5E7EB] shadow-xl relative overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                          <h2 className="text-xl font-bold">PlanificaciÃ³n Semanal con IA</h2>
                      ...
                      <div className="relative">
                        <textarea 
                          ...
                          placeholder="Ej: Esta semana quiero lanzar el nuevo dashboard, Elena se encargarÃ¡ del frontend y Lucas del SEO..."
                          value={plannerInput}
                ...
                <div className="lg:col-span-12">
                  ...
                  {suggestedActivities.length === 0 ? (
                    <div className="bg-white p-12 rounded-3xl border border-[#E5E7EB] text-center space-y-4">
                      ...
                      <div>
                        <h4 className="font-bold text-gray-800">No hay sugerencias aÃºn</h4>
                        <p className="text-sm text-gray-400 max-w-xs mx-auto">Escribe o graba tus planes arriba para ver cÃ³mo Gemini los organiza en tareas.</p>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              {settingsSubTab === 'general' && (
                <div className="bg-white p-12 rounded-[3rem] border border-[#E5E7EB] shadow-xl text-center space-y-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-black text-gray-900 mb-2">ConfiguraciÃ³n del Sistema</h2>
                    <p className="text-gray-500 max-w-sm mx-auto font-medium">Personaliza los parÃ¡metros globales de Novagreen IA y gestiona las integraciones de Gemini.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    ...
                    <div className="p-6 bg-gray-50 rounded-3xl border border-gray-100 text-left group hover:bg-white hover:shadow-md transition-all cursor-not-allowed opacity-60">
                      ...
                      <h4 className="font-bold text-gray-800 mb-1">Frecuencia de AnÃ¡lisis</h4>
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Tiempo Real (Streaming)</p>
                    </div>
                  </div>

                  <div className="pt-8 border-t border-gray-100">
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">MÃ³dulo de ConfiguraciÃ³n v1.0.4</p>
                  </div>
                </div>
              )}
                      ...
                      if (!member) {
                        return (
                          <div className="text-center py-20 text-gray-400 font-medium text-sm">
                            Por favor, agrega un miembro al equipo en la pestaÃ±a "Equipo" para poder configurar sus permisos.
                          </div>
                        );
                      }
                          ...
                          <div className="p-5 bg-slate-50 border border-slate-100 rounded-2xl space-y-4">
                            <div>
                              <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                                <Shield size={14} className={draftIsSystemAdmin ? 'text-red-500' : 'text-blue-500'} /> ConfiguraciÃ³n de Rol Global
                              </h4>
                              <p className="text-[11px] text-slate-400 mt-1 font-semibold leading-relaxed">
                                Determina si este integrante tiene privilegios ilimitados de administraciÃ³n en todo el sistema.
                              </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              ...
                              >
                                ...
                                <p className="text-[9px] font-bold leading-relaxed text-slate-400">
                                  Acceso total para hacer y deshacer. Control operativo en todos los mÃ³dulos sin restricciones.
                                </p>
                              </button>
                              ...
                              >
                                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                  ...
                                  <span className="font-extrabold text-[10px] uppercase tracking-wider">Acceso por MÃ³dulos</span>
                                </div>
                                <p className="text-[9px] font-bold leading-relaxed text-slate-400">
                                  Matriz de permisos granulados. El usuario puede tener privilegios Ãºnicos por mÃ³dulo del sistema.
                                </p>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="space-y-4">
                            <div>
                              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                <Lock size={12} className="text-blue-500" /> Matriz de Niveles de Acceso por MÃ³dulo
                              </h4>
                              <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
                                Define el perfil de permisos operativos (Ninguno, Lector, Colaborador, LÃ­der o Administrador) para cada Ã¡rea.
                              </p>
                            </div>
                            ...
                            {draftIsSystemAdmin ? (
                              <div className="p-6 bg-amber-50/50 border border-dashed border-amber-200 rounded-2xl text-center space-y-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-slate-500 max-w-sm mx-auto mt-1 leading-relaxed font-semibold">
                                    Este integrante posee acceso sin lÃ­mites para "hacer y deshacer". Todos los mÃ³dulos se encuentran asignados a nivel total.
                                  </p>
                                </div>
                              </div>
                            ) : (
                              <div className="border border-slate-200 bg-white rounded-xl overflow-hidden shadow-sm divide-y divide-slate-100">
                                ...
                                <div className="hidden md:grid md:grid-cols-12 gap-4 px-6 py-3.5 bg-slate-50/75 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                  <div className="col-span-5 flex items-center">MÃ³dulo o Ãrea del Sistema</div>
                                  <div className="col-span-7 flex justify-end pr-8">Nivel de Acceso Asignado</div>
                                </div>

                                {[
                                  { id: 'dashboard', name: 'Resumen o Dashboard', desc: 'Panel de control con mÃ©tricas generales del equipo.', icon: <TrendingUp size={16} /> },
                                  { id: 'gerencia', name: 'MÃ³dulo de Gerencia', desc: 'Asistente IA consultor, bitÃ¡cora directiva, planificaciÃ³n estratÃ©gica (FODA/OKRs) y gobernanza de IA.', icon: <Briefcase size={16} /> },
                                  { id: 'marketing', name: 'MÃ³dulo de Marketing', desc: 'CampaÃ±as estandarizadas [AAMMDD], proyectos vinculados, calendario de contenidos, CRM y mÃ©tricas.', icon: <Megaphone size={16} /> },
                                  { id: 'ventas', name: 'MÃ³dulo de Ventas', desc: 'CRM de Clientes, Embudo de Ventas (Pipeline), Cotizaciones y Metas.', icon: <DollarSign size={16} /> },
                                  { id: 'productos', name: 'MÃ³dulo de Productos', desc: 'CatÃ¡logo de soluciones comerciales, fichas tÃ©cnicas y precios de CertificaciÃ³n, CapacitaciÃ³n, QHSE, EPP y Equipos.', icon: <Boxes size={16} />, isProductosParent: true },
                                  { id: 'acreditacion', name: 'MÃ³dulo de AcreditaciÃ³n', desc: 'Enlaces, aliados y programas de acreditaciÃ³n internacional.', icon: <Award size={16} /> },
                                  { id: 'qhse', name: 'MÃ³dulo de QHSE', desc: 'GestiÃ³n de Calidad, Salud, Seguridad Ocupacional y Medio Ambiente.', icon: <ShieldCheck size={16} /> },
                                  { id: 'importaciones', name: 'MÃ³dulo de Importaciones', desc: 'Base de productos, catÃ¡logo de proveedores internacionales y Ã³rdenes de importaciÃ³n.', icon: <Package size={16} /> },
                                  { id: 'capacitacion', name: 'MÃ³dulo de CapacitaciÃ³n', desc: 'GestiÃ³n de capacitadores, aulas, lugares, calendario y liquidaciÃ³n de costos.', icon: <GraduationCap size={16} /> },
                                  { id: 'tasks', name: 'Seguimiento de Tareas', desc: 'Permisos de tareas gestionados individualmente para cada proceso especÃ­fico.', icon: <CheckCircle2 size={16} />, isTasksParent: true },
                                  { id: 'planner', name: 'Planificador Inteligente IA', desc: 'PlanificaciÃ³n inteligente asistida por modelos Gemini.', icon: <Calendar size={16} /> },
                                  { id: 'projects', name: 'GestiÃ³n de Proyectos', desc: 'AdministraciÃ³n de campaÃ±as y portafolio de proyectos.', icon: <FolderKanban size={16} />, isProjectsParent: true },
                                  { id: 'process_dashboard', name: 'GestiÃ³n XD (Procesos)', desc: 'AdministraciÃ³n y seguimiento de horas, enlaces, bases y minutas por proceso.', icon: <Activity size={16} />, isProcessDashboardParent: true },
                                  { id: 'directory', name: 'Directorio de Contactos', desc: 'InformaciÃ³n tÃ©cnica de contactos y ficha de integrante.', icon: <Contact size={16} /> },
                                  { id: 'transcript', name: 'Analizador de Minutas IA', desc: 'Procesamiento de audios de reuniones and minutas.', icon: <Sparkles size={16} /> },
                                  { id: 'settings', name: 'ConfiguraciÃ³n y Procesos', desc: 'Permisos de equipo y control administrativo general.', icon: <Settings size={16} /> },
                                ].map(mod => {
                                    ...
                                    <div key={mod.id} className="p-5 md:px-6 md:py-4 bg-white hover:bg-slate-50/20 transition-all">
                                        ...
                                        <div className="mt-3.5 border-t border-dashed border-slate-100 pt-3">
                                          <div className="flex items-center justify-between pb-1 flex-wrap gap-2">
                                            ...
                                            <span className="text-[8px] font-bold text-slate-400 italic">
                                              * Los integrantes heredarÃ¡n acceso a la secciÃ³n de Tareas de acuerdo al rol configurado en cada proceso.
                                            </span>
                                          </div>
                                              ...
                                              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center border-b border-dashed border-slate-200/60 pb-3">
                                                <div className="md:col-span-5 flex items-center gap-2">
                                                  ...
                                                  <div className="flex flex-col min-w-0">
                                                    ...
                                                    <span className="text-[8px] text-slate-400 font-bold leading-normal mt-0.5">Controla el acceso de las personas a tareas que no tienen un proceso o asignado especÃ­fico</span>
                                                  </div>
                                                </div>
                                                <div className="md:col-span-7 flex justify-end w-full">
                                      ...
                                      {mod.isProcessDashboardParent && (
                                        <div className="mt-3.5 border-t border-dashed border-slate-100 pt-3">
                                          <div className="flex items-center justify-between pb-1 flex-wrap gap-2">
                                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                                              <Sparkles size={11} className="text-blue-500" /> Permisos de GestiÃ³n XD por Proceso Activo
                                            </span>
                                            <span className="text-[8px] font-bold text-slate-400 italic">
                                              * Los integrantes heredarÃ¡n acceso a la secciÃ³n de GestiÃ³n XD de acuerdo al rol configurado en cada proceso.
                                            </span>
                                          </div>
                                      ...
                                      {mod.isProjectsParent && (
                                        <div className="mt-3.5 border-t border-dashed border-slate-100 pt-3">
                                          <div className="flex items-center justify-between pb-1 flex-wrap gap-2">
                                            ...
                                            <span className="text-[8px] font-bold text-slate-400 italic">
                                              * Los integrantes heredarÃ¡n acceso a la secciÃ³n de Proyectos de acuerdo al rol configurado en cada proceso.
                                            </span>
                                          </div>
                                      ...
                                      {(mod as any).isProductosParent && (
                                        <div className="mt-3.5 border-t border-dashed border-slate-100 pt-3">
                                          <div className="flex items-center justify-between pb-1 flex-wrap gap-2">
                                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                                              <Boxes size={11} className="text-blue-500" /> Permisos por SubmÃ³dulo de Productos
                                            </span>
                                            <span className="text-[8px] font-bold text-slate-400 italic">
                                              * Permite asignar permisos especÃ­ficos a cada categorÃ­a o heredar del permiso general de Productos.
                                            </span>
                                          </div>

                                          <div className="mt-3.5 pl-4 pr-1.5 py-3.5 bg-slate-50/50 border border-slate-200/50 border-dashed rounded-xl space-y-3">
                                            {[
                                              { id: 'certificacion', name: 'CertificaciÃ³n', icon: 'ðŸ…' },
                                              { id: 'capacitacion', name: 'CapacitaciÃ³n', icon: 'ðŸ“š' },
                                              { id: 'qhse', name: 'QHSE', icon: 'ðŸ›¡ï¸' },
                                              { id: 'epp', name: 'EPP', icon: 'ðŸ¦º' },
                                              { id: 'equipos', name: 'Equipos', icon: 'âš™ï¸' }
                                            ].map(sub => {
                                                ...
                                                <div key={sub.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                                                  <div className="md:col-span-5 flex items-center gap-2">
                                                    ...
                                                    <div className="flex items-center gap-1.5 min-w-0">
                                                      ...
                                                      <span className="text-[8px] bg-slate-200/50 text-slate-500 py-0.5 px-1.5 rounded font-black uppercase tracking-wider flex-shrink-0">SubmÃ³dulo</span>
                                                    </div>
                                                  </div>
                                                  <div className="md:col-span-7 flex justify-end w-full">
                                                    <div className="flex bg-slate-200/50 p-0.5 rounded-lg w-full max-w-sm gap-0.5">
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div>
                                  <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                                    <span>Enlaces de InterÃ©s: SupervisiÃ³n y Visibilidad</span>
                                    <span className="text-[9px] bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-bold">GestiÃ³n Centralizada</span>
                                  </h4>
                                  <p className="text-[10px] text-slate-500 font-semibold mt-0.5">
                                    Define de quÃ© otras personas puede ver y supervisar los enlaces de interÃ©s en todos los mÃ³dulos (ej: LÃ­deres de GestiÃ³n).
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            {draftIsSystemAdmin ? (
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-200/70 rounded-xl">
                                  <div>
                                    <h5 className="text-xs font-bold text-slate-800">Ver todos los enlaces de la organizaciÃ³n</h5>
                                    <p className="text-[10px] text-slate-400 font-medium">Permite ver los enlaces de interÃ©s de todas las personas sin restricciÃ³n.</p>
                                  </div>
                                  <button
                                    type="button"
                ...
                <div className="space-y-8">
                  <div className="flex items-center justify-between bg-white p-8 rounded-[3rem] border border-[#E5E7EB] shadow-xl">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-2xl font-black text-ng-black">GestiÃ³n de Procesos</h2>
                        <p className="text-ng-black/40 text-sm font-medium">Estructura operativa y departamentos de la organizaciÃ³n.</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => {
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between bg-white p-8 rounded-[3rem] border border-[#E5E7EB] shadow-xl">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-2xl font-black text-ng-black">GestiÃ³n de Equipo</h2>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1 bg-white">
                    <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Â¿Salir sin guardar permisos?</h3>
                    <p className="text-xs text-slate-500 font-bold leading-relaxed">
                      Has modificado los niveles de acceso para <span className="text-slate-800 font-black">{resolvedPermissionsMember?.name}</span> pero no has guardado los cambios. Si sales ahora, se perderÃ¡n de manera irreversible.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 flex items-center gap-2 border border-slate-100 text-[10px] text-slate-400 font-semibold uppercase tracking-wider">
                  ...
                  Selecciona una opciÃ³n para continuar con la navegaciÃ³n.
                </div>
            ...
            >
              ...
              >
                ...
                <form onSubmit={handleAddProcess} className="p-8 space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">DescripciÃ³n</label>
                    <textarea 
                      required
                      placeholder="Â¿De quÃ© se encarga este proceso?"
              ...
              >
                <form onSubmit={editingProject ? handleUpdateProject : handleAddProject} className="flex flex-col flex-1 overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Ciudad / UbicaciÃ³n (Opcional)</label>
                      <input 
                        type="text"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">DescripciÃ³n</label>
                      <textarea 
                    ...
                    <div className="flex items-center gap-4 flex-1 min-w-0 pr-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <input
                          ...
                          placeholder="TÃ­tulo de la historia de usuario..."
                    ...
                    <div className="lg:col-span-3 space-y-6">
                      <div className="bg-gray-50/50 p-4 rounded-3xl border border-gray-100 space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <option value="standard">Desarrollo / EstÃ¡ndar</option>
                            <option value="design_post">DiseÃ±o - Post EstÃ¡tico</option>
                            <option value="design_carousel">DiseÃ±o - Carrusel</option>
                            <option value="design_video">DiseÃ±o - Video</option>
                          </select>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <option value="">Sin Asignar (RevisiÃ³n de LÃ­der/Admin)</option>
                            {sortedMembers.map(m => (
                              <option key={m.id} value={m.id}>{m.name}</option>
                            ))}
                          </select>
                          ...
                          {showAddAuxDropdown && (
                            <div className="absolute right-0 top-12 w-64 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 p-3 space-y-2.5">
                              <div className="flex items-center justify-between border-b border-gray-100 pb-1.5">
                                <span className="text-[9px] font-black text-gray-400 uppercase tracking-wider">AÃ±adir Miembro Auxiliar</span>
                                <button
                                  type="button"
                                  onClick={() => {
                                    setShowAddAuxDropdown(false);
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            {(() => {
                              const allOptions = [
                                { value: 'backlog', label: 'ðŸ“¦ Product Backlog' },
                                { value: 'todo', label: 'ðŸ“‹ Por Hacer' },
                                { value: 'in_progress', label: 'âš¡ En Progreso' },
                                { value: 'blocked', label: 'ðŸš« Bloqueada' },
                                { value: 'review', label: 'ðŸ” En RevisiÃ³n' },
                                { value: 'correction', label: 'ðŸ”§ Para CorrecciÃ³n' },
                                { value: 'done', label: 'âœ… Completada' },
                                { value: 'rejected', label: 'âŒ Rechazada' }
                              ];
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <option value="baja">ðŸŸ¢ Baja (Normal)</option>
                            <option value="media">âš¡ Media (EstÃ¡ndar)</option>
                            <option value="alta">ðŸ”¥ Alta (Urgente)</option>
                            <option value="meteoric_crash">â˜„ï¸ Meteoric Crash (ALERTA MÃXIMA)</option>
                          </select>
                        </div>
                                ...
                                <div className="flex items-center justify-between gap-4">
                                  ...
                                  <div className="relative">
                                    {canEditMetadataField && (
                                      ...
                                      >
                                        <Plus size={10} strokeWidth={3} /> {showAddBlockerDropdown ? 'Cerrar' : 'AÃ±adir'}
                                      </button>
                                    )}
                                    ...
                                    {showAddBlockerDropdown && (
                                      <div className="absolute left-0 lg:left-full lg:-ml-4 mt-2 lg:mt-0 w-72 bg-white rounded-2xl border border-gray-200 shadow-xl p-3 z-50 space-y-2 animate-in fade-in zoom-in-95">
                                        ...
                                        {/* Filtros rÃ¡pidos: Proyecto y Proceso */}
                                ...
                                <div className="flex items-center justify-between gap-4">
                                  ...
                                  <div className="relative">
                                    {canEditMetadataField && (
                                      ...
                                      >
                                        <Plus size={10} strokeWidth={3} /> {showAddBlocksDropdown ? 'Cerrar' : 'AÃ±adir'}
                                      </button>
                                    )}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 space-y-3">
        ...
        {/* FILA 2: Fecha LÃ­mite | Horario Toggle */}
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          <div className={`space-y-2 relative flex-1 p-3 -m-3 rounded-xl border transition-colors ${!canEditPlanning ? 'bg-red-50/40 border-red-50/50' : 'bg-red-50/80 border-red-100'}`}>
            <label className="text-[10px] font-bold text-red-600 uppercase tracking-wider ml-1 flex items-center gap-1.5" title="Solo el lÃ­der de proceso o administrador puede cambiar esta fecha">
              <Calendar size={12} className="text-red-500" /> Fecha LÃ­mite
      ...
      <div className="bg-white p-4 rounded-2xl border border-blue-100 space-y-4 shadow-sm shadow-blue-900/5">
        <h4 className="text-[10px] font-black uppercase tracking-widest text-blue-600 flex items-center gap-2 mb-2">
          <CheckCircle2 size={12} className="text-blue-500" /> EjecuciÃ³n Real
        </h4>
    ...
    {/* Section: Split Description into Description and Acceptance Criteria */}
                            <div className="space-y-3">
                              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] ml-1">DescripciÃ³n de la historia</label>
                              <textarea 
                              ...
                              {!canEditStoryAndCriteria && (
                                <span className="text-[8px] font-black tracking-tight text-red-500 uppercase block pl-1">Solo LÃ­der / Administrador</span>
                              )}
                            </div>

                            <div className="space-y-3">
                              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] ml-1 flex items-center gap-2">
                                <CheckSquare size={14} className="text-emerald-500" /> Criterios de AceptaciÃ³n
                              </label>
                              <textarea 
                            ...
                            <div className="space-y-4 pt-6 border-t border-gray-100">
                              ...
                              {(newTaskData.deliverables && newTaskData.deliverables.length > 0) ? (
                                      ...
                                      <div className="flex-1 space-y-3">
                                        <div className="flex flex-col sm:flex-row gap-3">
                                          <div className="flex items-center gap-2 flex-1">
                                            ...
                                            <input
                                              type="text"
                                              placeholder="UbicaciÃ³n en Drive (Ruta o carpeta)..."
                                              disabled={!canEditExecution}
                                ...
                                <div className="text-center py-6 bg-gray-50/50 rounded-2xl border border-gray-100 border-dashed">
                                  <p className="text-xs font-medium text-gray-400">No hay links de entrega aÃ±adidos.</p>
                                </div>
                              )}
                            </div>
                                    ...
                                    <>
                                      {slides.map(slideIdx => {
                                        ...
                                        return (
                                          <div key={slideIdx} className="space-y-3">
                                            <div className="flex items-center justify-between">
                                              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] ml-1 flex items-center gap-2">
                                                <Layers size={14} className="text-purple-500" /> Elementos del DiseÃ±o {newTaskData.taskTemplate === 'design_carousel' ? `- Imagen ${slideIdx}` : ''}
                                              </label>
                                              <div className="flex items-center gap-2">
                                              ...
                                              <table className="min-w-full w-max text-left text-xs table-fixed">
                                                ...
                                                <tbody className="divide-y divide-gray-100 bg-white">
                                                  {slideElements.length === 0 && (
                                                    <tr>
                                                      <td colSpan={5} className="px-4 py-8 text-center text-gray-400 italic">No hay elementos agregados. AÃ±ade una fila para comenzar.</td>
                                                    </tr>
                                                  )}
                                      ...
                                      {newTaskData.taskTemplate === 'design_carousel' && (
                                        <div className="flex justify-start">
                                          ...
                                          >
                                            <Plus size={14} /> AÃ±adir imagen al carrusel
                                          </button>
                                        </div>
                                      )}
                                    </>
                                              ...
                                              <thead className="bg-gray-50 text-gray-500 uppercase text-[10px] font-black tracking-wider border-b border-gray-100">
                                                <tr>
                                                  ...
                                                  <th style={{ width: videoColWidths.stage }} className="p-0 border-r border-gray-100/50 relative group select-none">
                                                    ...
                                                    <div className="absolute right-0 top-0 bottom-0 w-1 bg-gray-200 opacity-0 group-hover:opacity-100 cursor-col-resize hover:bg-purple-400 transition-colors" onMouseDown={(e) => { const startX = e.pageX; const startWidth = videoColWidths.stage; const onMouseMove = (moveEvent) => setVideoColWidths(prev => ({ ...prev, stage: Math.max(50, startWidth + (mxœì]ënã6þ¿OÁzÚÆF¶ã$³¨ÇÉ`03ÛfÑNƒôAAK´Í‰$ª$•KÓ<L ?û l^lII–äKtñtc¬ü#cI$Ežs¾ï™gØ5ywM|ÙðŒüˆ,$$æòÇn=t»/‘Í|!ó¿a¡ ßèuºèøÝ#‡Ù¡§:râ±h¯©Ä'¼ÓöT{u¾½÷þŽ`ÄýÂ`Ñë» ú<¤ºaÇ)¯5–oô€ú'B¥?ã¾œWê'ç ð;—ßß£êÈù]S‡°7ÌýAŠÞ5!vÕÌlñ{ä¸X4aÜ!Üâñ—ÇwÖþ`Ð? N\,é5A3ÎÂ 	â[Z>óI«Ê4a¢½ÎLàÖ:DÁu€¦.¹ETOX6H˜p²çS—ÝXsê8Äo|o¡C½Çßf ~úˆ8Ô¦ÿò»ã>½Iá‰`n(	ât6— "ÉŒ ¤d|¹±öÑdf5°Må\Ñb²æjæ£ø,ˆÙ!Œ[6s-Ný… ÓF	B¸Ä:„V’c_PI™¯Z2.Z±R½e7þñ}‡D6c¬ÉX˜1F÷2}^¯:\[©9{TZ®,2±#}Aä÷™Î€“ku¥sz½ž:ÚCfÀúËyÏÃ·£Á^zIÛø†gê˜ÿÁæ„øçäVî´‡PÀñQ€}‰]£õsH$nD!‘V…-º‰ô°³ØigqÍ¨M¾åÜiOñ=ûEù	66~¡qˆW}›Ü!³ñ;íØD~•þ	ôë¯H©ö.û†oÍóØð<D4î¡mXVm²‡Ôè¯x¾¾âÞ'7çX\½Å÷PÈ™¯¾¾êÙ¡ W:ô|ñ
V0è€:êµ®f^°æWäîøFéQçá	7uaš]nÛC™3 ±Š®j¥³*g5\á¬>‚$éôÎšyCÉ;¯Šs€YP?%ªÜ¡kì†Ä,‡òPc(æ¿™cFÎ©Æ`(²{ÐNÐV®¥S\Y•Ê\\v/_naÔ¹…	DSéM©ïœú¹íØê)mPPt||ŒŒ®vëÝNQGÝîäºñ-/àÌ¥^íáÁ=Îˆìé¥«w7påï5î;%â=´ñå¯¦à³=´¼£dñÀ“×™èCµLé5]Wáµ†× s0ÐØsP„ÛX$!äµ.öÁí%š2_ZÛW
”í+êÏ,pV`×º•v8Gà›¦àéÅˆ«ËXû­Ês®äÿÍg<	˜øu<˜±Kí+°âO`ÄeÀfJ]ðž)#ûladÆºåê“Rþ…¶€¶Î©Ïqâh^I]#3ì5ÅK1˜’çÿ­¥ìI*´Þ¹Ô£>æj‰A‚¸úÕÍèœcÑðòãûýaE2Õ7ÖX™cTcþŸ™IÜbÜÄ/Ÿ>~©Át»U<¤ŠDVð÷eIXáæÃŠª©z°,ï˜¢£ª[Ü¬'~
´µÁD~jƒ ’žÏn:5Àn;”L‹:#3Å=¤hóµß‡ä#Ó·ÑCò°Ä¡2iÈ„×û)Š{˜%‘‡‘œ]x}y³$'ÏÖ‡ÆÑ¡!èUU<"¯ÿ‰Z“GT†Ý371}8¬á«S‡j~zñ²½ÔvJw“æÜ¥U¸
˜€u‡¢/±»UŒäfSÁï–îp¿ÖÕh¾ñÁV?_yÕs‰?ìxµ&–I7F#ta|QÛ!SºÒê@¦¤Ú+µ5›˜E_ãf´Ûù·¯úêâÕK[9°®NÇuôˆÐxÐgWg&ºU3tcÉM~Nª3t©e2y´ÄÞ£Ð·T¥“w8›òyØ[À[«gí”ï‚¨WöýœÝ°ìê1Yàb›Ì™1ÏÇŒ@Ï-õÏQõ§x’#•¦¦ç=è<€F†Ü›¼i.u‰&.³¯ªO;ÉþBä÷Q	èöÁ‹à¶]'ìådªâ
÷d1m•U‹Žï‘þÒÓ·îÍõMÁ¡dí—«/F'mÎ\×LOfùRtõ‰2ÿTåoSAZÃÕ‚›ßºYnn¶tyåÌ«Ï;J¡<Yl1iœðÇÈý>Á ÓžzÙâä¾_|	ÓÐ$0rÔÆóg³¸žÕ¸ñùhJ$£” ëå°jYÈ†x3ÑÐÇ%š/Ÿã‰KôÕAX ìßåPpµ•°Û•âCõ©È–æDQÏGèKìÛsÖ`aƒ…;‡…šË7`ˆ
ƒaü4hXèSµ´8ÜtÃç‡§ÉÎ‡ú¿yÔÆ”¶ÁÇwM‚«HT ã”`ƒ…>uÒˆ»ÈM7|~ùŸŸs<ÁaU‰êúªÇwÓo|ˆD…!2û¦¬ÊBŸº@™z—›nøüàòž<þûsHš@²ÁÊ]ÄÊä·P¢â±äâ$JúÔ'c‰7¹é†Ï"3[‰€l r2¿¹»Ýn)3›×°,ô©R¦„¾CxùÇn 7øœÙBÿ‡‚õ6àz€ƒìZ(½³8ý©z‡°zwÑ:‡×ÆYü×7‘ÞÒX¹?]Q†ÿ	zÆï´4#í,ËYMéþ¡›Þç™‡úz`ÿüá¾æöòšmæ¯>ƒÊ›³«Ã}µÝ¦ËauR/ÆìÁ×n%çµûO«ïµI6¡Vö6v¡.íCÑŠ|ÎÞ—€²ÄjhE–õŽÖïbŒK$[šÌ‰À-wV`|v“cUÉÔ d™’Õ÷Ö-XPÕ¦ªì<¬b‡pµï°Ü­ “rde:•,ÛPªy‰Æ Ÿ'Ûå‹D ßµ‰¥vÁÒ:Œ¬´>?ö“u±
:¶±‹'Ä]ªl²¢ZOÆêSÅ}â*>ƒÞñ.M“åéÍ0ð÷ÂÞv|êOYÊóó¨¥|MÌ&À-m
Àh*¤QìáûúéJjÕ
”|ª²…ªDâ†‘F\ýmB˜ÖÖÒ¤ÙØ0œ1;k§Œ{X
s 0¤
^ÀÑÅåžŠ„ôJ}\Ž&d–;¡ÊK3í-n‘+`°(©Ð“ìƒTf.*ÔOÕ€eýä‚Zµ¿Ò7ÛC!öÛÌSí.3íòœué™²r‹%PŽª– ¦À¢ÅÖä\‰®ÖoÝÜ&æý,-Ðç¾X®vdÜ`Þ"1ÇõÂ+®ð¹R_Ã*‚X‘fÎEG)1zÜØ9½(ÁÊ1‚Âè5,Š"…n¾"‰cÖiq€&'³|Ã”V€ZÖƒê*X´±\…ò¥(³~iw|^‡¢ªI£zˆ¤Fe>7¤Ùf1U`þ[Žgê5cé(ŸèJGÚvu}‚âÕ[Ôf'œq`Â3³-Ó5›ZÑ+§ê
©"Cv´>1`‚/k«â"æ°]ð.½‡Ý×_—Ï<3Ù™bP¼øXp|Åo£F~©N˜ö Ãså  UACÀøÏ?G«Î_.»%ˆáª§ÎU®¯8ÖªÉª^=Å-zº~™øÊy§M=<#ýv·ÌÜ Þø]ÉD£yõßq	ß`*QÀ0ñÚwÞ0´EˆS5=×’Y¯gÌ?5SÔk¸D#Y×aŠÏ†+ª„\XÚs°:ÎËik¼ :3Þi½SÿÄêÒDZ†£,;Œ^6ß
á—ÙA±ï0äb3®ßC§À|Ð:˜
}„¹=§×y¿	äÒá¬×*+­‹·-Ú² ‚&¢¹XöÆÚ¢¹úÓ·á­›”‹ZPò§kŽe¢ó5¿DÎ5] lX&W.T.Å,¥#nE”*a®“Š’þªžõ5çX mEwH„Ä•JK“=¨$1WŠþ9|ü}ÜjM%ÊÜdÓ5ž´½£ÖÉ03øûÙ—{èì=üùLÎJÜ²„X‹G%Þú§|4 ýša‹Fq9±4Pí‹þ( o3N¤þ¨˜E@È#¼Ñâð yNêð0
\Y"´ö‘Ó£’Á±öé§Nù:dú¹ô¯` UópÎ•É¨å*û§¢­ö›Žñ”“ÈGZA\´?‰ùÊåÈ"é¨	+ÀÕµàcÌ}UúgcêÍà¶‘  õÀ‡<n-R†­¡îÜ"6ù¨þƒ[=`*×Ñ*ûB ‹Få§½äŸóJgYR)˜bž9ö^{UZåT!jwœR¾sÉ7ÕÞËÕyW»ì³&Ó‹	.u2œïü´‡¨Õ%ç#WV~ÏˆWxÏ¿ªv¸ª~U?@æåjÌ¹ú_DéVÎÉ¤[ÝYŠ-^µÆÍR‰WCÞê¼P-mÞúåé°zÁ÷*ïLKú”â¯:Ë½ÜÚx…š=qÃñF¹oP÷‡ngíÐ&¶öÒz¸âÿüy¡~Ã±þEi.7¼ñˆïD¤LÌ¹ŠEÖáÝf°(
…?¸¯SñÚq üU.«3Å®Ø”uöï0Û¨¹ø¿¶õZo”ï‘þ‰8™mØ¾d~Ë‘,B²NùÞ`p½.ækth“íY*N<*×-Õ
]Ë	ÓŸY.õ¢ÿ³R/¿)éS‘^xã¨gx´ÚÅŸ#âàË§kw–\Jšã7s¢fòôOUÆ"ÀþÉ=Y(péök[†Ø¥¿`Ž¾¢B2NqÈjû'éSã¾î^zÑÖ8…q_‘•üùqßcJ<½¥>ë®¤üÔ¸ÿÚ‡A’3N„‚÷¸]j
0¦zžÉI0¨‡?ý  ÿÿ  "ä¡